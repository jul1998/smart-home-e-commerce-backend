...
def add_item_to_shopping_cart(user_id):
    ...
    if ShoppingCart.query.filter_by(productId=product_id).first() and ShoppingCart.query.filter_by(user_id=user_id).first():
...
def detelete_product_shopping_cart(user_id):
    ...
    if not ShoppingCart.query.filter_by(productId=product_id).first() and not ShoppingCart.query.filter_by(user_id=user_id).first():
        raise APIException("Product does not exist in shopping cart from user")
  
    product_to_delete = ShoppingCart.query.filter_by(user_id=user_id).first()
